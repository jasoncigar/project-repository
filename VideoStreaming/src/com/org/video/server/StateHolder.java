...
public class StateHolder {
	...
	private CellbotProtos.ControllerState.Builder csBuilder = CellbotProtos.ControllerState
			.newBuilder();
	...
	public void setPhoneState(CellbotProtos.PhoneState ps) {
		...
		if (phoneState != null && MERGE_PHONE_STATE) {
			...
			phoneState = CellbotProtos.PhoneState.newBuilder(phoneState)
					.mergeFrom(ps).build();
		} else {
			phoneState = ps;
		}
	...
	public long getavTimeStamp() {
		return avFrame.getTimestamp();
	}
	...
	public int addKeyEvent(
			com.cellbots.CellbotProtos.ControllerState.KeyEvent.Builder key) {
		if (csBuilder == null)
